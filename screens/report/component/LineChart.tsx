...
const LineChart: React.FC<LineChartProps> = ({
  ...
  const pointSpacing = svgWidth / data.length;
  const maxValue =
    Math.max(...yAxisValues.map(y => parseFloat(y.value)), 1) * 1.4;
      ...
      <G>
        ...
        {yAxisValues.map((y, i) => (
          <SvgText
            key={i}
            x="20"
            y={chartHeight - (parseFloat(y.value) / maxValue) * chartHeight}
            fontSize="12"
            fill="#888"
            textAnchor="middle">
            {y.label}
